...
public class UserController {
    ...
    public Result<?> register(@RequestBody User user){
        User res = userMapper.selectOne(Wrappers.<User>lambdaQuery().eq(User::getNum,user.getNum()));
        if(res != null)
        {
            return Result.error("-1","身份已重复");
        }
    ...
    public Result<?> login(@RequestBody User user){
        ...
        User resById = userMapper.selectById(res.getId());
    ...
    public Result<?> findPage(@RequestParam(defaultValue = "1") Integer pageNum,
        ...
        if(StringUtils.isNotBlank(search)){
            wrappers.like(User::getNum,search);
        }
    ...
    public Result<?> findPage2(@RequestParam(defaultValue = "1") Integer pageNum,
        ...
        if(StringUtils.isNotBlank(search2)){
            wrappers.like(User::getNum,search2);
        }
